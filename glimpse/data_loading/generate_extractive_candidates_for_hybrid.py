...
from glimpse.data_loading.generate_extractive_candidates import evaluate_summarizer as extractive_summarizer
...
def parse_args():
    ...
    parser.add_argument("--extractive_decoding_config", type=str, default="top_p_sampling",
                        help="Decoding strategy for extractive summarization.")
    parser.add_argument("--batch_size", type=int, default=16, help="Batch size for summarization.")
    parser.add_argument("--device", type=str, default="cuda", help="Device for summarization.")
...
def main():
    ...
    # Load model and tokenizer for extractive summarization
    extractive_model = AutoModelForSeq2SeqLM.from_pretrained(args.abstractive_model_name).to(args.device)
    extractive_tokenizer = AutoTokenizer.from_pretrained(args.abstractive_model_name)
    extractive_dataset = extractive_summarizer(
        model=extractive_model,
        tokenizer=extractive_tokenizer,
        dataset=dataset,
        decoding_config=args.extractive_decoding_config,
    ...
    extractive_df = extractive_df.drop(columns=["text"])
